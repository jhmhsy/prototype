<div class="w-full max-w-6xl mx-auto px-4 md:px-6 lg:px-8 py-12 md:py-16 lg:py-20">
    <div class="grid gap-12 md:gap-16 lg:gap- text-textblack dark:text-textwhite">
        <section>
            <div class="flex items-center justify-between mb-6 md:mb-8">
                <h2 class="text-2xl md:text-3xl font-bold">Gym Equipment</h2>
                <a class="hover:underline" href="#">View all</a>
            </div>

            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8 lg:gap-10">
                <div
                    class="bg-muted rounded-lg p-4 md:p-6 flex flex-col items-center text-center bg-white dark:bg-darkmode_light">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                        class="w-10 h-10 mb-4 text-black dark:text-white">
                        <path d="M14.4 14.4 9.6 9.6"></path>
                        <path
                            d="M18.657 21.485a2 2 0 1 1-2.829-2.828l-1.767 1.768a2 2 0 1 1-2.829-2.829l6.364-6.364a2 2 0 1 1 2.829 2.829l-1.768 1.767a2 2 0 1 1 2.828 2.829z">
                        </path>
                        <path d="m21.5 21.5-1.4-1.4"></path>
                        <path d="M3.9 3.9 2.5 2.5"></path>
                        <path
                            d="M6.404 12.768a2 2 0 1 1-2.829-2.829l1.768-1.767a2 2 0 1 1-2.828-2.829l2.828-2.828a2 2 0 1 1 2.829 2.828l1.767-1.768a2 2 0 1 1 2.829 2.829z">
                        </path>
                    </svg>
                    <h3 class="text-lg font-medium mb-2">Free Weights</h3>
                    <x-input-p class="text-muted-foreground text-sm">Variety of dumbbells and barbells for strength
                        training.</x-input-p>
                </div>
                <div
                    class="bg-muted rounded-lg p-4 md:p-6 flex flex-col items-center text-center bg-white dark:bg-darkmode_light">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                        class="w-10 h-10 mb-4 text-black dark:text-white">
                        <line x1="10" x2="14" y1="2" y2="2"></line>
                        <line x1="12" x2="15" y1="14" y2="11"></line>
                        <circle cx="12" cy="14" r="8"></circle>
                    </svg>
                    <h3 class="text-lg font-medium mb-2">Cardio Equipment</h3>
                    <x-input-p class="text-muted-foreground text-sm">Treadmills, ellipticals, and stationary bikes
                        for
                        cardio.</x-input-p>
                </div>
                <div
                    class="bg-muted rounded-lg p-4 md:p-6 flex flex-col items-center text-center bg-white dark:bg-darkmode_light">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                        class="w-10 h-10 mb-4 text-black dark:text-white">
                        <path d="M12 3a6 6 0 0 0 9 9 9 9 0 1 1-9-9Z"></path>
                    </svg>
                    <h3 class="text-lg font-medium mb-2">Stretching Area</h3>
                    <x-input-p class="text-muted-foreground text-sm">Dedicated space for yoga, Pilates, and other
                        stretching.</x-input-p>
                </div>
                <div
                    class="bg-muted rounded-lg p-4 md:p-6 flex flex-col items-center text-center bg-white dark:bg-darkmode_light">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                        class="w-10 h-10 mb-4 text-black dark:text-white">
                        <rect width="18" height="18" x="3" y="3" rx="2"></rect>
                        <path d="M3 12h18"></path>
                    </svg>
                    <h3 class="text-lg font-medium mb-2">Rowing Machines</h3>
                    <x-input-p class="text-muted-foreground text-sm">High-intensity rowing machines for a full-body
                        workout.</x-input-p>
                </div>
            </div>
        </section>
        <section>
            <div class="flex items-center justify-between mb-6 md:mb-8">
                <h2 class="text-2xl md:text-3xl font-bold">Gym Facilities</h2>
                <a class="text-primary hover:underline" href="#">
                    View all
                </a>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8 lg:gap-10">
                <div
                    class="bg-muted rounded-lg p-4 md:p-6 flex flex-col items-center text-center bg-white dark:bg-darkmode_light">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                        class="w-10 h-10 mb-4 text-black dark:text-white">
                        <path d="m4 4 2.5 2.5"></path>
                        <path d="M13.5 6.5a4.95 4.95 0 0 0-7 7"></path>
                        <path d="M15 5 5 15"></path>
                        <path d="M14 17v.01"></path>
                        <path d="M10 16v.01"></path>
                        <path d="M13 13v.01"></path>
                        <path d="M16 10v.01"></path>
                        <path d="M11 20v.01"></path>
                        <path d="M17 14v.01"></path>
                        <path d="M20 11v.01"></path>
                    </svg>
                    <h3 class="text-lg font-medium mb-2">Locker Rooms</h3>
                    <x-input-p class="text-muted-foreground text-sm">Spacious locker rooms with showers and changing
                        areas.</x-input-p>
                </div>
                <div
                    class="bg-muted rounded-lg p-4 md:p-6 flex flex-col items-center text-center bg-white dark:bg-darkmode_light">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                        class="w-10 h-10 mb-4 text-black dark:text-white">
                        <path d="M21.66 17.67a1.08 1.08 0 0 1-.04 1.6A12 12 0 0 1 4.73 2.38a1.1 1.1 0 0 1 1.61-.04z">
                        </path>
                        <path d="M19.65 15.66A8 8 0 0 1 8.35 4.34"></path>
                        <path d="m14 10-5.5 5.5"></path>
                        <path d="M14 17.85V10H6.15"></path>
                    </svg>
                    <h3 class="text-lg font-medium mb-2">Juice Bar</h3>
                    <x-input-p class="text-muted-foreground text-sm">Refuel with a variety of healthy smoothies and
                        juices.</x-input-p>
                </div>
                <div
                    class="bg-muted rounded-lg p-4 md:p-6 flex flex-col items-center text-center bg-white dark:bg-darkmode_light">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                        class="w-10 h-10 mb-4 text-black dark:text-white">
                        <path d="M3.5 21 14 3"></path>
                        <path d="M20.5 21 10 3"></path>
                        <path d="M15.5 21 12 15l-3.5 6"></path>
                        <path d="M2 21h20"></path>
                    </svg>
                    <h3 class="text-lg font-medium mb-2">Towel Service</h3>
                    <x-input-p class="text-muted-foreground text-sm">Complimentary towel service for your workout
                        needs.
                    </x-input-p>
                </div>
                <div
                    class="bg-muted rounded-lg p-4 md:p-6 flex flex-col items-center text-center bg-white dark:bg-darkmode_light">
                    <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none"
                        stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                        class="w-10 h-10 mb-4 text-black dark:text-white">
                        <path d="M12 20h.01"></path>
                        <path d="M2 8.82a15 15 0 0 1 20 0"></path>
                        <path d="M5 12.859a10 10 0 0 1 14 0"></path>
                        <path d="M8.5 16.429a5 5 0 0 1 7 0"></path>
                    </svg>
                    <h3 class="text-lg font-medium mb-2">Free WiFi</h3>
                    <x-input-p class="text-muted-foreground text-sm">Stay connected with high-speed wireless
                        internet
                        access.</x-input-p>
                </div>
            </div>
        </section>
        <section>
            <div class="flex items-center justify-between mb-6 md:mb-8">
                <h2 class="text-2xl md:text-3xl font-bold">Gym Rooms</h2>
                <a class="text-primary hover:underline" href="#">
                    View all
                </a>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 md:gap-8 lg:gap-10">
                <div class="bg-muted rounded-lg p-4 md:p-6 flex flex-col items-center text-center">
                    <img src="/placeholder.svg" width="200" height="150" alt="Group Fitness" class="rounded-lg mb-4"
                        style="aspect-ratio: 200 / 150; object-fit: cover;" />
                    <h3 class="text-lg font-medium mb-2">Group Fitness</h3>
                    <x-input-p class="text-muted-foreground text-sm">Variety of group classes like yoga, Pilates,
                        and HIIT.</x-input-p>
                </div>
                <div class="bg-muted rounded-lg p-4 md:p-6 flex flex-col items-center text-center">
                    <img src="/placeholder.svg" width="200" height="150"
                        style="aspect-ratio: 200 / 150; object-fit: cover;" alt="Personal Training"
                        class="rounded-lg mb-4" />
                    <h3 class="text-lg font-medium mb-2">Personal Training</h3>
                    <x-input-p class="text-muted-foreground text-sm">Work one-on-one with our certified personal
                        trainers.</x-input-p>
                </div>
                <div class="bg-muted rounded-lg p-4 md:p-6 flex flex-col items-center text-center">
                    <img src="/placeholder.svg" width="200" height="150" alt="Basketball Court" class="rounded-lg mb-4"
                        style="aspect-ratio: 200 / 150; object-fit: cover;" />
                    <h3 class="text-lg font-medium mb-2">Basketball Court</h3>
                    <x-input-p class="text-muted-foreground text-sm">Full-size basketball court for pickup games and
                        le
                    </x-input-p>
                </div>
                <div class="bg-muted rounded-lg p-4 md:p-6 flex flex-col items-center text-center">
                    <img src="/placeholder.svg" width="200" height="150" alt="Spin Studio" class="rounded-lg mb-4"
                        style="aspect-ratio: 200 / 150; object-fit: cover;" />
                </div>
            </div>
        </section>



    </div>
</div>