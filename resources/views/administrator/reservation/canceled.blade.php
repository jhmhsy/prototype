...
border-shade_6/50 dark:border-white/5" data-v0-t="card">
        <div class="flex flex-col gap-2">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <!-- Pagination -->
                <div>
                    {{ $rejectedBookings->links('vendor.pagination.custom-pagination') }}
                </div>


                <!-- Search bar -->
                <form method="GET" action="{{ route('administrator.suspended') }}" class="mb-4">
                    <div class="relative">
                        <button type="submit"
                ...
                <thead class="text-black dark:text-white">
                    <tr class="transition-colors hover:bg-muted/50">
                        <th class="h-12 px-4 text-left align-middle">
                            <a href="{{ route('administrator.suspended', ['sortBy' => 'name', 'sortDirection' => $sortBy === 'name' && $sortDirection === 'asc' ? 'desc' : 'asc', 'search' => request('search')]) }}"
                                class="{{ $sortBy === 'name' ? 'text-black dark:text-white' : 'text-gray-500' }}">
                                Name
                            </a>
                        </th>
                        <th class="h-12 px-4 text-left align-middle">
                            <a href="{{ route('administrator.suspended', ['sortBy' => 'email', 'sortDirection' => $sortBy === 'email' && $sortDirection === 'asc' ? 'desc' : 'asc', 'search' => request('search')]) }}"
                                class="{{ $sortBy === 'email' ? 'text-black dark:text-white' : 'text-gray-500' }}">
                                Email
                            </a>
                        </th>
                        <th class="h-12 px-4 text-left align-middle">
                            <a href="{{ route('administrator.suspended', ['sortBy' => 'date', 'sortDirection' => $sortBy === 'date' && $sortDirection === 'asc' ? 'desc' : 'asc', 'search' => request('search')]) }}"
                                class="{{ $sortBy === 'date' ? 'text-black dark:text-white' : 'text-gray-500' }}">
                                Date
                            </a>
                        </th>
                        <th class="h-12 px-4 text-left align-middle">
                            <a href="{{ route('administrator.suspended', ['sortBy' => 'time', 'sortDirection' => $sortBy === 'time' && $sortDirection === 'asc' ? 'desc' : 'asc', 'search' => request('search')]) }}"
