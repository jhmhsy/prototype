    ...
    border-shade_6/50 dark:border-white/5">
            ...
            <div class="flex flex-col gap-2">
                <div class="flex justify-between">
                    <h1 class="text-xl font-bold dark:text-white">Tickets Info</h1>

                </div>


                <span class="text-sm text-gray-600 dark:text-gray-400" @if ($tickets->isEmpty()) style="display:
                    none;" @endif>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <!-- Search bar -->
                    <form method="GET" action="{{ route('administrator.tickets') }}" class="mb-4">
                        <div class="relative">
                            <button type="submit"
                        ...
                        <thead class="text-black dark:text-white">
                            <tr class="border-b transition-colors hover:bg-muted/50 data-[state=selected]:bg-muted">
                                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">
                                    <a href="{{ route('administrator.tickets', ['sortBy' => 'name', 'sortDirection' => $sortBy === 'name' && $sortDirection === 'asc' ? 'desc' : 'asc', 'search' => request('search')]) }}"
                                        class="{{ $sortBy === 'name' ? 'text-black dark:text-white' : 'text-gray-500' }}">
                                        Name
                                    </a>
                                </th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">
                                    <a href="{{ route('administrator.tickets', ['sortBy' => 'email', 'sortDirection' => $sortBy === 'email' && $sortDirection === 'asc' ? 'desc' : 'asc', 'search' => request('search')]) }}"
                                        class="{{ $sortBy === 'email' ? 'text-black dark:text-white' : 'text-gray-500' }}">
                                        Email
                                    </a>
                                </th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">
                                    <a href="{{ route('administrator.tickets', ['sortBy' => 'quantity', 'sortDirection' => $sortBy === 'quantity' && $sortDirection === 'asc' ? 'desc' : 'asc', 'search' => request('search')]) }}"
                                        class="{{ $sortBy === 'quantity' ? 'text-black dark:text-white' : 'text-gray-500' }}">
                                        Quantity
                                    </a>
                                </th>
                                <th class="h-12 px-4 text-left align-middle font-medium text-muted-foreground">
                                    <a href="{{ route('administrator.tickets', ['sortBy' => 'status', 'sortDirection' => $sortBy === 'status' && $sortDirection === 'asc' ? 'desc' : 'asc', 'search' => request('search')]) }}"
                        ...
                        <tbody class="text-gray-600 dark:text-gray-400">
                            ...
                            <tr>
                                <td colspan="6" class="text-center py-4">
                                    <span class="text-gray-600 dark:text-gray-400">No Booked Ticket yet.</span>
                                </td>
                            </tr>
                            @else
